...
ENVIRONMENT_NAME="$(echo "$ENVIRONMENT_NAME_FOR_HUMANS" | tr "[:upper:]" "[:lower:]" | tr -s -c "[:alnum:]-" "-" | sed 's/--*/-/g;s/^-//;s/-$//')"
...
echo ""
echo "You can enter the environment by simply cd'ing into this directory:"
echo ""
